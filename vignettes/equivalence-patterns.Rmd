...
**Scope note:** Visual equivalence is purely output-based — it checks that two
plots produce the same rendered data, labels, facets, and coordinate system.  It
does not verify data provenance.  If two different datasets happen to produce
visually identical output (same bar heights, same x-axis labels), visual
equivalence will pass.  Use structural equivalence when you need to verify that
the same data and the same code idiom were used.
...
|---|---|---|---|---|
| **(a)** | — | ✓ (heights + label both "count") | ✓ (col name "count" = stat default) | ✓ (explicit label matches) |
| **(b)** | ✓ | — | ✓ | ✓ |
| **(c)** | ✓ | ✓ | — | ✓ |
| **(d)** | ✓ | ✓ | ✓ | — |
